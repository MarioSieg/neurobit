...
static inline uint8_t saturate_u4(int32_t x) { return x < 0 ? 0 : x > 0xF ? 0xF : x; }
static inline uint8_t saturate_u2(int32_t x) { return x < 0 ? 0 : x > 3 ? 3 : x; }
...
void neurobit_quant_float_to_uint4(const float* in, uint8_t* out, size_t len, float scale, int32_t zero_point) {
    ...
    size_t i;
    for (i=0; i+1 < len; i += 2) {
        uint8_t a = saturate_u4(zero_point + (int32_t)roundf(in[i]*inv_scale));
        uint8_t b = saturate_u4(zero_point + (int32_t)roundf(in[i+1]*inv_scale));
        out[i>>1] = a | b<<4;
    }
    if (len & 1) /* Odd tail */
        out[i>>1] = saturate_u4(zero_point + (int32_t)roundf(in[len-1]*inv_scale));
}
...
void neurobit_quant_float_to_uint2(const float* in, uint8_t* out, size_t len, float scale, int32_t zero_point) {
    ...
    size_t i;
    for (i=0; i+3 < len; i += 4) {
        uint8_t a = saturate_u2(zero_point + (int32_t)roundf(in[i]*inv_scale));
        uint8_t b = saturate_u2(zero_point + (int32_t)roundf(in[i+1]*inv_scale));
        uint8_t c = saturate_u2(zero_point + (int32_t)roundf(in[i+2]*inv_scale));
        uint8_t d = saturate_u2(zero_point + (int32_t)roundf(in[i+3]*inv_scale));
    ...
    if (len & 3) { /* Handle 1-, 2- or 3-value tail */
        uint8_t p = 0;
        switch (len & 3) {
            case 3: p |= saturate_u2(zero_point + (int32_t)roundf(in[i+2]*inv_scale)) << 4;
            case 2: p |= saturate_u2(zero_point + (int32_t)roundf(in[i+1]*inv_scale)) << 2;
            case 1: p |= saturate_u2(zero_point + (int32_t)roundf(in[i]*inv_scale));
        }
        out[i>>2] = p;
    }
}
